...
for i in get_address_late():
    ...
    count = requests.get("http://172.168.10.22:5000/table/count")
    count = count.text
    ...
    alarm =(''' 尊敬的%s单位，您好!
        ...
        (总销项任务数：%d件  已完成任务数：%d件 滞后任务数：%d件)''')% (i[1],data,event_list,count_all,count_complete,count_late)
...
for j in get_address_near():
    ...
    event_list = str(get_address_near()[i])
    alarm =(''' 尊敬的%s单位，您好!
